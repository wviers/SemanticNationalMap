...
class Reader:
    ...
    def __shape(self):
        ...
        elif shapeType in (3,5,8,10,13,15,18,19,23,25,28,31):
            record.bbox = _Array('d', unpack("<4d", f.read(32)))
        # Shape types with parts
        if shapeType in (3,5,10,13,15,19,23,25,31):
            nParts = unpack("<i", f.read(4))[0]
        # Shape types with points
        if shapeType in (3,5,8,10,13,15,19,23,25,31):
        ...
        if shapeType in (10,13,15,18,19,31):
        ...
        # Read a single point
        if shapeType in (1,9,11,21):
            record.points = [_Array('d', unpack("<2d", f.read(16)))]
        # Read a single Z value
        if shapeType in (9,11):
